...
import { CreateCarDTO, CarDTO, CarResponseDTO } from './car-settings';
import { CarResponse } from './car-setting.model';

@Injectable()
export class HttpCarController implements CarController {
  private readonly BASE_URL = `http://localhost:5000/api/Cars`;
  public defaultHeaders = new HttpHeaders();
  public encoder: HttpParameterCodec;
  constructor(private httpClient: HttpClient) {}
  public createCar(request: CreateCarDTO): Observable<CarResponseDTO> {
    return this.httpClient
      .post(`${this.BASE_URL}/add`, request)
      .pipe(map((res: CarResponse) => res));
  }
  public editCar(request: CarDTO): Observable<CarDTO> {
    return this.httpClient
      .put(`${this.BASE_URL}/${request._id}`, request)
      .pipe(map((res: CarDTO) => res));
  }
